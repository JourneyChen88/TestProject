...
namespace ConsoleApp1
{
    ...
    public static class SortExtention
    {
         ...
         * 若a[1]大于a[2]则交换两者的值，否则不变。
         * 再比较a[2]与a[3]的值，若a[2]大于a[3]则交换两者的值，否则不变。再比较a[3]与a[4]，
         * 以此类推，最后比较a[n-1]与a[n]的值。
         * 这样处理一轮后，a[n]的值一定是这组数据中最大的。再对a[1]~a[n-1]以相同方法处理一轮，
         ...
         * 优点：稳定
         * 时间复杂度：理想情况下（数组本来就是有序的），此时最好的时间复杂度为o(n),
         ...
         * 多个相同的值的相对位置也许会在算法结束时产生变动。
         一趟快速排序的算法是：
            ...
            即3中A[j]不小于key,4中A[i]不大于key的时候改变j、i的值，
         ...
         * 插入排序的基本方法是：每步将一个待排序的记录按其关键字的大小插到前面已经排序的序列中的适当位置，
         * 直到全部记录插入完毕为止。
         */
        /// <summary>
        /// 直接插入排序
        ...
        public static void InsertSort(int[] arry)
        {
            ...
            for (int i = 1; i < arry.Length; i++)
            {
                ...
                if (arry[i] < arry[i - 1])
                {
                    //用一个变量来保存当前待比较的数值，因为当一趟比较完成时，
                    //我们要将待比较数值置入比它小的数值的后一位 
                    int temp = arry[i];
                    int j = 0;
        ...
        public static void HeapSort(int[] arry, int top)
        {
        ...
        public static void RadixSort(int[] arry, int array_x = 10, int array_y = 100)
        {
