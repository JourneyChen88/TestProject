        ...
        private string GetEntityCode(TreeNode node)
        {
            ...
            switch (type)
            {
                case DataBaseType.SQLServer:
                    ...
                    codeString.Append($@"{(string.IsNullOrWhiteSpace(this.Using_TextBox.Text) ? "" : $"{Environment.NewLine}{this.Using_TextBox.Text.Trim()}")}
...
{{
    ...
    {{");
                    ...
                    DataTable colsInfos = SQLServerHelper.QueryDataTable(node.Parent.Name, $@"
                                        SELECT
                                        B.name AS OBJNAME,
                                        C.value AS VALUE
                                        FROM sys.tables A
                                        ...
                                        WHERE A.name = '{node.Text}'");

                    //SELECT OBJNAME,VALUE FROM ::FN_LISTEXTENDEDPROPERTY (NULL, 'USER', 'DBO', 'TABLE', '" + node.Text + "', 'COLUMN', DEFAULT)", null);
...
");
        ...
        private void 字符转属性ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ToClassForm tc = new ToClassForm();
            tc.ShowDialog();
        }
    }
}